...
def analyze_data_quality(df: pd.DataFrame, city_name: str = "Unknown") -> dict:
    ...
    if 'owner_email' in df.columns:
        ...
        analysis['email_completion_rate'] = (analysis['records_with_email'] / analysis['total_records']) * 100

    if 'owner_phone' in df.columns:
        analysis['records_with_phone'] = df['owner_phone'].notna().sum()
        analysis['phone_completion_rate'] = (analysis['records_with_phone'] / analysis['total_records']) * 100
    ...
    if 'priority_flag' in df.columns:
        analysis['priority_records'] = df['priority_flag'].sum() if df['priority_flag'].dtype == bool else 0
...
def export_to_google_sheets(df: pd.DataFrame, sheet_name: str, analysis: dict) -> bool:
    ...
    try:
        ...
        try:
            ...
            worksheet.update(f'A{summary_row}', f'Summary - Total Records: {analysis["total_records"]}')
            worksheet.update(
                f'A{summary_row + 1}',
                f'Quality Score Average: {analysis.get("avg_quality_score", "N/A")}'
            )
            worksheet.update(f'A{summary_row + 2}', f'Export Date: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}')
...
def generate_processing_summary(data: dict, analyses: list, api_status: dict, export_results: dict) -> dict:
...
def main():
    ...
    print(f"Google Sheets exports: {sum(1 for success in export_results.values() if success)}/{len(export_results)}")
