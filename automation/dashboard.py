...
class DashboardManager:
    ...
    def save_status(self, status: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if "global_stats" in status:
                ...
                last_run = status["global_stats"].get("last_run", datetime.now().isoformat())
                status["global_stats"]["last_run"] = str(last_run)
            with open(self.status_file, "w") as f:
    ...
    def _create_html_content(self, status: Dict[str, Any]) -> str:
        ...
        for site_name, site_data in sites.items():
            ...
            status_color = {"success": "#28a745", "failed": "#dc3545", "running": "#ffc107", "unknown": "#6c757d"}.get(
                site_status, "#6c757d"
            )

            status_icon = {"success": "✅", "failed": "❌", "running": "🔄", "unknown": "❓"}.get(site_status, "❓")
