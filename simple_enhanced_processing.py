...
def load_and_process_data():
    ...
    if hallandale_file.exists():
        print(f"✅ Loading Hallandale data...")
...
def generate_final_summary(results, found_pdfs):
    ...
    # Add recommendations
    total_exported = sum(1 for city_data in results.values() if city_data.get('export_success', False))
    ...
    for city, data in results.items():
        ...
        if 'email_records' in data:
            report_lines.append(f"  • Email completion: {data['email_records']}/{data['total_records']}")
        if 'phone_records' in data:
            report_lines.append(f"  • Phone completion: {data['phone_records']}/{data['total_records']}")
    ...
    report_lines.extend([
        "",
        f"Additional PDFs found: {len(found_pdfs)}",
        "",
        "=" * 60
    ])
