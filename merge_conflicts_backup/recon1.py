...
=======
    filename=os.path.join(OUTPUT_DIR, LOG_FILE), level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
>>>>>>> 3ccf4fd (Committing all changes)
)
...
=======
                By.XPATH, "//*[contains(text(), 'Next') or contains(text(), 'Load More')]"
>>>>>>> 3ccf4fd (Committing all changes)
            )
            if not next_buttons:
                break
