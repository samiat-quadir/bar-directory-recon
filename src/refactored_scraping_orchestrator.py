...
class IndustryScrapingOrchestrator:
    ...
    def _handle_exports(
        ...
        if export_format in ["google_sheets", "both"]:
            ...
            if (
                not sheets_result["google_sheets_uploaded"]
                and export_format == "google_sheets"
                and not export_results["csv_path"]
            ):
    ...
    def _build_error_result(self, error_message: str) -> dict[str, Any]:
        ...
        return {"success": False, "error": error_message, "leads": [], "count": 0}

    def _build_success_result(
        self,
        enriched_leads: list[dict],
        industry: str,
        target_plugins: list[dict],
        ...
        return {
            ...
            "industry": industry,
            "output_path": (
                str(export_results["csv_path"]) if export_results["csv_path"] else None
            ),
