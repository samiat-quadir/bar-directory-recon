...
PATTERNS = [
    ...
    (re.compile(r'(requests\.(get|post|put|delete|head|patch)\([^\)]*?),\s*timeout\s*=\s*\d+([^\)]*?),\s*timeout\s*=\s*\d+(\s*\))'), r'\1\3\4'),
    (re.compile(r'(subprocess\.run\([^\)]*?),\s*timeout\s*=\s*\d+([^\)]*?),\s*timeout\s*=\s*\d+(\s*\))'), r'\1\2\3'),
    ...
    (re.compile(r'(,\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*[^,\)]+)(?:(?:,\s*\2\s*=\s*[^,\)]+)+)'), lambda m: m.group(1)),
]
