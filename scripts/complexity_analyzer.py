    ...
    def identify_refactor_candidates(
        ...
        for func in functions:
            ...
            if score >= 2:  # Threshold for refactoring
                candidates.append({**func, "refactor_score": score, "reasons": reasons})
...
def main():
    ...
    print(
        f"   Functions with complexity > 10: {len([f for f in all_functions if f['complexity'] > 10])}"
    )
    print(
        f"   Functions with complexity > 7: {len([f for f in all_functions if f['complexity'] > 7])}"
    )
