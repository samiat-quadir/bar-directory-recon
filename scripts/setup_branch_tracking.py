...
class BranchTrackingManager:
    ...
    def fix_all_branches(self) -> bool:
        ...
        success, output = self.run_command(
            ["git", "branch", "--format=%(refname:short)"], "Getting all local branches"
        )
...
def main():
    ...
    parser.add_argument(
        "--current", action="store_true", help="Set up tracking for current branch only"
    )
    parser.add_argument(
        "--all", action="store_true", help="Fix tracking for all local branches"
    )
    parser.add_argument(
        "--create-remote",
        action="store_true",
    ...
    parser.add_argument("--branch", help="Specific branch to set up tracking for")
