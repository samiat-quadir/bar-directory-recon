...
class HVACPlumberScraper:
    ...
    def scrape_test_data(self) -> List[Dict[str, str]]:
        ...
        sample_names = [
            "Mike Johnson", "Sarah Wilson", "Dave Rodriguez", "Lisa Thompson",
            "Robert Kim", "Amanda Martinez", "Chris Davis", "Jennifer Garcia",
            "James Miller", "Maria Lopez", "Daniel Brown", "Emily Taylor"
        ]

        sample_businesses = [
            "Arctic Air HVAC", "Precision Plumbing", "CoolBreeze Systems", "FlowMaster Plumbing",
            "Climate Control Pros", "DrainMaster Services", "TempRight HVAC", "PipeDream Plumbing",
            "AirFlow Solutions", "H2O Plumbing Experts", "Comfort Zone HVAC", "Reliable Pipe Works"
        ]

        service_types = [
            "HVAC Installation", "Plumbing Repair", "Air Conditioning", "Water Heater Service",
            "Heating Systems", "Drain Cleaning", "Ductwork", "Emergency Plumbing",
            "HVAC Maintenance", "Pipe Installation", "Furnace Repair", "Leak Detection"
        ]
    ...
    def scrape_live_data(self) -> List[Dict[str, str]]:
        ...
        for url in target_urls:
            try:
                ...
                listing_selectors = [
                    '.business-name', '.contractor-card', '.service-provider',
                    '.listing-item', '.business-listing', '.company-info',
                    '[data-business]', '[data-contractor]', '.hvac-listing',
                    '.plumber-listing'
                ]
                ...
                for i, listing in enumerate(listings[:self.max_records//len(target_urls)]):
                    try:
                        ...
                        business_name = (name_elem.get_text(strip=True) if name_elem
                                       else f"HVAC/Plumbing Contractor {i+1}")

                        # Determine service type from business name or text
                        service_type = "General HVAC/Plumbing"
                        if any(keyword in text.lower() for keyword in ['hvac', 'heating', 'cooling', 'air']):
                            service_type = "HVAC Services"
                        elif any(keyword in text.lower() for keyword in ['plumbing', 'plumber', 'pipe', 'drain']):
                            service_type = "Plumbing Services"

                        contractor = {
                            "Full Name": "Service Manager",  # Often not available in directory listings
                            "Email": contact_info["email"] or f"contact@{business_name.lower().replace(' ', '')}.com",
