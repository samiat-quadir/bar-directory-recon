...
from universal_recon.plugins.firm_parser import parse_firm_data
...
class TestFirmParserPlugin(unittest.TestCase):
    ...
    def test_plugin_output_soft_mode(self):
        records = parse_firm_data(self.driver, context="test_soft")
...
def test_parse_firm_data():
    # Basic test to ensure the module loads
    assert callable(parse_firm_data)
