"""
Notification Agent - Phase 4 Optimize Prime - ASUS Device Version
    ...
    def _prepare_notification_content(
        ...
        for i, lead in enumerate(urgent_leads[:5], 1):  # Show top 5 urgent leads
            ...
            if email:
                summary += f" | {email}"
            if phone:
                summary += f" | [Phone: {self._sanitize_phone_number(phone)}]"
...
"""
        ...
        try:
            ...
            for phone_number in self.config.to_phone_numbers or []:
                try:
                    ...
                    logger.error(
                        f"Failed to send SMS to {self._sanitize_phone_number(phone_number)}: {e}"
                    )
    ...
    def _channel_configured(self, channel: str) -> bool:
        ...
        if channel == "email":
            ...
            return bool(
                self.config.to_phone_numbers and self.config.twilio_account_sid and SMS_AVAILABLE
            )
        elif channel == "slack":
