...
try:
    from tools.realtor_directory_scraper import (
        scrape_realtor_directory as enhanced_scraper,
    )
    from universal_recon.plugins.realtor_directory_plugin import (
        scrape_realtor_directory as plugin_scraper,
    )
...
Examples:
  ...
  python realtor_automation.py --schedule
    ...
    parser.add_argument(
        "--test", action="store_true", help="Run a test scrape with limited records"
    )
...
def show_scheduling_instructions() -> None:
    ...
    print(
        "  2. Add: 0 8 * * 1 /path/to/python /path/to/realtor_automation.py --output /path/to/leads.csv"
    )
    print()
